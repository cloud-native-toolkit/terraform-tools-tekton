...
if [[ -n "${CSV}" ]]; then
  DEPLOYMENTS=$(oc get deployment -n "${NAMESPACE}" -l "${DEPLOYMENT_LABEL}" -o json | jq -r '.items[] | .metadata.name')
fi
...
if [[ -n "${SUBSCRIPTION}" ]]; then
  ...
  echo "The OpenShift Pipelines CSV is deployed but there is no subscription" >&2
  exit 1
else
  echo '{"exists": "false"}'
  exit 0
